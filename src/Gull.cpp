...
GHandle SHARED = NULL, HASH = NULL;

#ifndef W32_BUILD
#define SET_BIT(var,bit) (builtin_sync_fetch_and_or(&(var),1 << (bit)))
#define SET_BIT_64(var,bit) (builtin_sync_fetch_and_or(&(var),Bit(bit)));
#define ZERO_BIT_64(var,bit) (builtin_sync_fetch_and_and(&(var),~Bit(bit)));
#define TEST_RESET_BIT(var,bit) (builtin_sync_bit_test_and_reset(&(var),bit))
#define TEST_RESET(var) (builtin_sync_lock_test_and_set(&(var),0))
#else
#define SET_BIT(var,bit) (builtin_sync_fetch_and_or(&(var),1 << (bit)))
#define SET_BIT_64(var,bit) {if ((bit) < 32) builtin_sync_fetch_and_or((LONG*)&(var),1 << (bit)); else builtin_sync_fetch_and_or(((LONG*)(&(var))) + 1,1 << ((bit) - 32));}
#define ZERO_BIT_64(var,bit) {if ((bit) < 32) builtin_sync_fetch_and_and((LONG*)&(var),~(1 << (bit))); else builtin_sync_fetch_and_and(((LONG*)(&(var))) + 1,~(1 << ((bit) - 32)));}
#define TEST_RESET_BIT(var,bit) (builtin_sync_bit_test_and_reset(&(var),bit))
#define TEST_RESET(var) (builtin_sync_lock_test_and_set(&(var),0))
#endif
#define SET(var,value) (builtin_sync_lock_test_and_set(&(var),value))

#define LOCK(lock) {while (builtin_sync_val_compare_and_swap(&(lock),1,0)) _mm_pause();}
...
#ifndef W32_BUILD
		builtin_sync_fetch_and_add(&Smpi->nodes, (long long)(nodes)-(long long)(check_node_smp));
#else
